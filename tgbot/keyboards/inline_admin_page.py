...
def position_edit_swipe_fp(remover: int, category_id: int) -> InlineKeyboardMarkup:
    ...
    for count, select in enumerate(range(remover, len(get_positions))):
        if count < 10:
            ...
            # Если используются размеры — показываем сумму остатков по размерам,
            # иначе старую логику по Itemx.
            ...
            keyboard.row(
                ikb(
                    f"{position.position_name} | {position.position_price} {CURRENCY_SYMBOL} | {qty}шт",
