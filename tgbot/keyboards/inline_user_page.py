...
def prod_item_position_swipe_fp(remover: int, category_id: int) -> InlineKeyboardMarkup:
    ...
    for count, select in enumerate(range(remover, len(get_positions))):
        if count < 10:
            ...
            # Если у позиции заведены размеры, считаем остаток по размерам.
            # Иначе используем старую логику по количеству товаров в таблице storage_item.
            ...
            keyboard.row(
                ikb(
                    f"{position.position_name} | {position.position_price} {CURRENCY_SYMBOL} | {total_qty} шт",
