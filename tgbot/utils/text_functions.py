...
try:
    ...
    try:
        from tgbot.data.config import CURRENCY as CURRENCY_SYMBOL
    except Exception:
        CURRENCY_SYMBOL = "сум"

from tgbot.database import (
    Categoryx, Positionx, Itemx, Purchasesx, PurchasesModel, Refillx, RefillModel, Settingsx,
    Userx, UserModel
)
...
# ✅ Формат денег (UZS) — чтобы не было 10.0 и т.п.
def money(value) -> str:
    try:
        v = float(value)
        if v.is_integer():
            return f"{int(v)} {CURRENCY_SYMBOL}"
...
async def open_profile_user(bot: Bot, user_id: Union[int, str]):
    ...
    count_items = sum(p.purchase_count for p in get_purchases)
    sum_amount = sum(p.purchase_price for p in get_purchases)

    send_text = ded(f"""
        <b>👤 Ваш профиль</b>
        ➖➖➖➖➖➖➖➖➖➖
        🆔 ID: <code>{get_user.user_id}</code>
        🎁 Бонусы: <code>{money(get_user.user_balance)}</code>
        🎁 Куплено товаров: <code>{count_items}шт</code>
        💰 Сумма заказов: <code>{money(sum_amount)}</code>
...
async def position_open_user(bot: Bot, user_id: int, position_id: Union[str, int], remover: Union[str, int]):
    ...
    send_text = ded(f"""
        ...
        ▪️ Стоимость: <code>{money(get_position.position_price)}</code>
        {text_desc}
    """)
...
async def open_profile_admin(bot: Bot, user_id: int, get_user: UserModel):
    ...
    send_text = ded(f"""
        ...
        ▪️ Бонусы: <code>{money(get_user.user_balance)}</code>
        ▪️ Всего пополнено: <code>{money(get_user.user_refill)}</code>
        ▪️ Куплено товаров: <code>{count_items}шт</code>
        ▪️ Сумма заказов: <code>{money(sum_amount)}</code>
    """)
...
async def refill_open_admin(bot: Bot, user_id: int, get_refill: RefillModel):
    ...
    send_text = ded(f"""
        ...
        ▪️ Сумма пополнения: <code>{money(get_refill.refill_amount)}</code>
...
async def purchase_open_admin(bot: Bot, arSession: ARS, user_id: int, get_purchase: PurchasesModel):
    ...
    send_text = ded(f"""
        ...
        ▪️ Цена одного товара: <code>{money(get_purchase.purchase_price_one)}</code>
        ▪️ Сумма покупки: <code>{money(get_purchase.purchase_price)}</code>

        ▪️ Баланс до покупки: <code>{money(get_purchase.user_balance_before)}</code>
        ▪️ Баланс после покупки: <code>{money(get_purchase.user_balance_after)}</code>
...
async def category_open_admin(bot: Bot, user_id: int, category_id: Union[str, int], remover: int):
    ...
    send_text = ded(f"""
        ...
        💸 Продаж за День: <code>{profit_count_day}шт</code> - <code>{money(profit_amount_day)}</code>
        💸 Продаж за Неделю: <code>{profit_count_week}шт</code> - <code>{money(profit_amount_week)}</code>
        💸 Продаж за Месяц: <code>{profit_count_month}шт</code> - <code>{money(profit_amount_month)}</code>
        💸 Продаж за Всё время: <code>{profit_count_all}шт</code> - <code>{money(profit_amount_all)}</code>
    """)
...
def get_statistics() -> str:
    ...
    return ded(f"""
        ...
        <b>💰 Продажи</b>
        ┣‒ Кол-во и сумма
        ┣ За День: <code>{profit_count_day}шт</code> - <code>{money(profit_amount_day)}</code>
        ┣ За Неделю: <code>{profit_count_week}шт</code> - <code>{money(profit_amount_week)}</code>
        ┣ За Месяц: <code>{profit_count_month}шт</code> - <code>{money(profit_amount_month)}</code>
        ┣ За Всё время: <code>{profit_count_all}шт</code> - <code>{money(profit_amount_all)}</code>
