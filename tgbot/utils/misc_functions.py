...
from tgbot.data.config import get_admins, BOT_VERSION, PATH_DATABASE, get_desc, CURRENCY_SYMBOL
        ...
        if len(get_positions) >= 1:
            ...
            for position in get_positions:
                if len(cache_items) < 30:
                    # Остаток: сначала пробуем размеры, иначе – старые товары
                    sizes = ItemSizex.gets(position.position_id)
                    if sizes:
                        total_qty = sum(int(s.qty) for s in sizes)
                    else:
                        get_items = Itemx.gets(position_id=position.position_id)
                        total_qty = len(get_items)

                    if total_qty >= 1:
                        cache_items.append(
                            f"{position.position_name} | {position.position_price} {CURRENCY_SYMBOL} | В наличии {total_qty} шт",
                        )
                else:
                    save_items.append("\n".join(cache_items))
                    cache_items = [f"<b>➖➖➖ {category.category_name} ➖➖➖</b>"]
...
def get_positions_items(category_id: Union[str, int]) -> list[PositionModel]:
    ...
    if get_settings.misc_hide_position == "True":
        for position in get_positions:
            # Остаток позиции считаем так:
            # 1) если есть размеры – по сумме qty по размерам
            # 2) иначе – по количеству строк в storage_item (старая логика)
            sizes = ItemSizex.gets(position.position_id)
            if sizes:
                total_qty = sum(int(s.qty) for s in sizes)
            else:
